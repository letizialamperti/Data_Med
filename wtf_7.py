...
def process_file(directory, filename, reference_df, unique_sample_dataframes, forward_file, reverse_file):
    ...
    try:
        ...
        for forward_filename, reverse_filename in zip([forward_file], [reverse_file]):
            with gzip.open(forward_filename, 'rt') as forward_handle, gzip.open(reverse_filename, 'rt') as reverse_handle:
