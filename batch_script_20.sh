...
piuttosto che avere una funzione che cerca tramite locate_excel_file(directory) il file Corr_tags, mi piacerebbe passarlo in input assieme al nome del file 

ChatGPT
ChatGPT
Per modificare il codice in modo da consentire di passare direttamente il percorso del file Excel contenente i tag corretti come argomento della funzione main(), è necessario apportare alcune modifiche al codice.

Ecco come puoi modificare il codice per consentire l'input del percorso del file Excel insieme al nome del file:

Modifica la firma della funzione main() per accettare due argomenti aggiuntivi: excel_file_path e directory_name.
Rimuovi la chiamata a locate_excel_file() all'interno di main(), poiché ora il percorso del file Excel verrà passato direttamente.
Modifica il flusso di esecuzione in modo che il percorso del file Excel venga passato alla funzione load_metadata().
Ecco come potrebbe apparire la funzione main() dopo le modifiche:

python
Copy code
def main(excel_file_path, directory_name):
    setup_logging()
...
Con queste modifiche, puoi eseguire il programma passando il percorso del file Excel come primo argomento e il nome della directory come secondo argomento. Ad esempio:

bash
Copy code
python nome_script.py /percorso/del/file_excel.xlsx nome_directory
Assicurati di aver installato tutti i pacchetti necessari e che il file Excel contenga i dati di riferimento corretti.
User
You
ora devo modificare il corrispettivo sh file che chiama il file python con 2 argomenti dati in ingresso:
...
ChatGPT
ChatGPT
Per modificare lo script bash in modo che chiami lo script Python con due argomenti dati in ingresso, devi passare i due argomenti direttamente allo script Python.

Ecco come puoi modificare lo script bash:
...
# Here you can just put the srun command you would execute. Just replace test_main.py with your script you want to run
# Make sure to replace <your conda environment> below with the name of the conda environment you want to use
srun -ul $HOME/miniconda3/envs/zioboia/bin/python wtf_20.py "$1" "$2" -u
